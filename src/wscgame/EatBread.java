...
public class EatBread extends JPanel {
    ...
    static BufferedImage Energy00Background;
    static BufferedImage Energy01Background;
    static BufferedImage Energy02Background;
    static BufferedImage Energy03Background;
    static BufferedImage Energy04Background;
    static BufferedImage Energy05Background;
    static MusicPlayer introMusic;
    static MusicPlayer backgroundMusic;
    static MusicPlayer deathMusic;
    static MusicPlayer jumpMusic;
    static MusicPlayer getMusic;
    ...
    static {
        try {
            ...
            Energy00Background = ImageIO.read(EatBread.class.getResource("E0.png"));
            Energy01Background = ImageIO.read(EatBread.class.getResource("E1.png"));
            Energy02Background = ImageIO.read(EatBread.class.getResource("E2.png"));
            Energy03Background = ImageIO.read(EatBread.class.getResource("E3.png"));
            Energy04Background = ImageIO.read(EatBread.class.getResource("E4.png"));
            Energy05Background = ImageIO.read(EatBread.class.getResource("E5.png"));
            ...
            deathMusic = new MusicPlayer(EatBread.class.getResource("death.wav"));
            jumpMusic = new MusicPlayer(EatBread.class.getResource("jump.wav"));
            getMusic = new MusicPlayer(EatBread.class.getResource("get.wav"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private DropBread[] droppingBread = {};    //一堆面包
     ...
     * 生成面包对象
     */
    public DropBread generateBread() {
            ...
            public void mouseClicked(MouseEvent e) {
                switch (state) {                                //根据当前状态做不同的操作
                    ...
                    case GAME_OVER:                             //游戏结束状态时
                        score = 0;                              //清理现场
                        wang = new Wang();
                        droppingBread = new DropBread[0];
                        state = START;                          //修改为启动状态
                        jumpMusic.play();
                        break;
                    case RUNNING:
                        if(e.isMetaDown()&&EnergyCount>0)
                        {
        ...
        timer.schedule(new TimerTask() {
            public void run() {
                if (state == RUNNING) {                       //运行状态时执行
                    ...
                    if(isPerformed){                            // 技能持续时间
                        int Duration = 500;
                        count++;
                        if(EnergyCount>0){
                            if((Duration-count)%(Duration/5)==0){
                                EnergyCount -= 1;
                            }
                        }
                        if(EnergyCount==0){
    ...
    public void paintScoreAndLives(Graphics g) {
        ...
        g.drawString(" X " + wang.getLives(), 280, 70);
    }
    ...
    public void paintEnergy(Graphics g){
        switch(EnergyCount){
            case 1:
                g.drawImage(Energy01Background,0,0,null);
                break;
            case 2:
                g.drawImage(Energy02Background,0,0,null);
                break;
            case 3:
                g.drawImage(Energy03Background,0,0,null);
                break;
            case 4:
                g.drawImage(Energy04Background,0,0,null);
                break;
            case 5:
                g.drawImage(Energy05Background,0,0,null);
                break;
            default:
                g.drawImage(Energy00Background,0,0,null);
                break;
        }
    }
    ...
    public void enterAction() {
        flyIndex++;
        if (flyIndex % 80 == 0) {
            DropBread obj = generateBread();                                             //获取面包对象
            //Consider ArrayList<>
    ...
    public void stepAction() {
        wang.step();                                                            //王思聪走一步
        for (DropBread movable : droppingBread) {                                 //遍历所有面包
                movable.step();                                                     //面包走一步
                if(isPerformed){ {
                    movable.moveTo(mousex);
                }
            }
        }
    ...
    public void outOfBoundsAction() {
        ...
        DropBread[] visibleBread = new DropBread[droppingBread.length];        //不越界面包数组
        for (DropBread f : droppingBread) {                   //遍历所有面包
    ...
    public void hitAction() {
        for (int i = 0; i < droppingBread.length; i++) {                         //遍历所有面包
            DropBread f = droppingBread[i];                                        //获取每一个面包
            if (wang.hit(f)) {                                                   //撞上了
                //交换被撞的面包与数组中的最后一个元素
                DropBread t = droppingBread[i];
